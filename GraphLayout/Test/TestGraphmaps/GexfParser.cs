...
namespace TestGraphmaps {
    internal class GexfParser {
        ...
        Graph graph;
        Dictionary<string, GexfNodeAttribute> idsToGexfNodeAttr=new Dictionary<string, GexfNodeAttribute>();
        ...
        void ReadEdge() {            
            graph.AddEdge(GetAttribute("source"), GetAttribute("target"));
            do {
                Read();
        ...
        void ReadNodeFeatures(Node node) {
            GexfNodeAttribute gexfNodeAttribute;
            idsToGexfNodeAttr[node.Id] = gexfNodeAttribute = new GexfNodeAttribute();
            while (xmlReader.IsStartElement()) {
                switch (xmlReader.Name) {
                    case "viz:color":
                        ...
                        break;
                    case "viz:position":
                        ReadPosition(gexfNodeAttribute);
                        break;
                    case "viz:size":
                        ReadSize(gexfNodeAttribute);
                        break;
                    default:
                        xmlReader.Skip();
                        break;
        ...
        void ReadSize(GexfNodeAttribute gexfNodeAttr) {
            var sizeVal = xmlReader.GetAttribute("value");
            if (sizeVal != null)
                gexfNodeAttr.Size = double.Parse(sizeVal);
        }

        void ReadPosition(GexfNodeAttribute gexfNodeAttr) {
            var xStr = GetAttribute("x");
            if (xStr != null) {
                double x;
                if (double.TryParse(xStr, out x)) {
                    var yStr = GetAttribute("y");
                    if(yStr!=null) {
                        double y;
        ...
        void ReadColor(Node node) {
            var r = byte.Parse(GetAttribute("r"));
            var g = byte.Parse(GetAttribute("g"));
            var b = byte.Parse(GetAttribute("b"));
            node.Attribute.Color = new Color(r, g, b);
        }
    }
}