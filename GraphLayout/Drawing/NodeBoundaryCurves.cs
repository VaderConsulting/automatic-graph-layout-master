  ...
  public sealed class NodeBoundaryCurves
  {
    ...
    public  static ICurve GetNodeBoundaryCurve(Node node, double width, double height)
    {
      ...
      switch (NodeAttribute.Shape)
      {
        ...
        case Shape.Box:
              if (NodeAttribute.XRadius != 0 || NodeAttribute.YRadius != 0)
                  return CurveFactory.CreateRectangleWithRoundedCorners(width, height, NodeAttribute.XRadius,
                                                                       NodeAttribute.YRadius, new P2(0, 0));
        ...
        default:
          {
            //  Console.WriteLine("creating ellipse for shape {0}",NodeAttribute.Shape);
            return new Ellipse(
