...
*/
        ...
        void TheDefaultObjectDecorator(IViewerObject obj) {
            ...
            if (node != null) {
                ...
                var w = drawingNode.Attribute.LineWidth;
                decoratorRemovalsDict[node] = (() => drawingNode.Attribute.LineWidth = w);
                drawingNode.Attribute.LineWidth = (int) Math.Max(viewer.LineThicknessForEditing, w*2);
            } else {
                var edge = obj as IViewerEdge;
                if (edge != null) {
                    var drawingEdge = edge.Edge;
                    var w = drawingEdge.Attribute.LineWidth;
                    decoratorRemovalsDict[edge] = (() => drawingEdge.Attribute.LineWidth = w);
                    drawingEdge.Attribute.LineWidth = (int) Math.Max(viewer.LineThicknessForEditing, w*2);
                }
            }
...
//        }
        ...
        bool NeedToCreateBoundaryPort(Point mousePoint, IViewerNode node, out double portParameter) {
            ...
            if (length <= viewer.UnderlyingPolylineCircleRadius*2 + drawingNode.Attribute.LineWidth/2) {
            ...
            if (SelectedEdge != null && SelectedEdge.Edge.GeometryEdge.UnderlyingPolyline!=null) {
                ...
                do {
                    if (MouseScreenPointIsCloseEnoughToVertex(site.Point,
                                                              SelectedEdge.RadiusOfPolylineCorner +
                                                              SelectedEdge.Edge.Attribute.LineWidth/2.0)) {
                        PolylineVertex = site;
                        e.Handled = true;
                        break;
                    }
        ...
        public Tuple<Site, PolylineCornerType> AnalyzeInsertOrDeletePolylineCorner(Point point, double tolerance) {
            ...
            tolerance += SelectedEdge.Edge.Attribute.LineWidth;
