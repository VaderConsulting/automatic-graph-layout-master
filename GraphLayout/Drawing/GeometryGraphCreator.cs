...
// && graph.LayoutAlgorithmSettings is SugiyamaLayoutSettings &&
            ...
            msaglEdge.Weight = drawingEdge.Attribute.Weight;
            msaglEdge.Length = drawingEdge.Attribute.Length;
            msaglEdge.Separation = drawingEdge.Attribute.Separation;
            if (drawingEdge.Attribute.ArrowAtSource) {
                msaglEdge.EdgeGeometry.SourceArrowhead = new Arrowhead {Length = drawingEdge.Attribute.ArrowheadLength};
            }
            if (drawingEdge.Attribute.ArrowAtTarget) {
                msaglEdge.EdgeGeometry.TargetArrowhead = new Arrowhead {Length = drawingEdge.Attribute.ArrowheadLength};
            }
            msaglEdge.UserData = drawingEdge;
            msaglEdge.LineWidth = drawingEdge.Attribute.LineWidth;
            return msaglEdge;
        }
        ...
        void ProcessPhyloEdges(Graph graph, GeometryGraph msaglGraph) {
            foreach (Edge e in graph.Edges) {
                ...
                msaglEdge.Weight = e.Attribute.Weight;
                msaglEdge.Separation = e.Attribute.Separation;
                if (e.Attribute.ArrowAtSource) {
                    msaglEdge.EdgeGeometry.SourceArrowhead = new Arrowhead {Length = e.Attribute.ArrowheadLength};
                }
                if (e.Attribute.ArrowAtTarget) {
                    msaglEdge.EdgeGeometry.TargetArrowhead = new Arrowhead {Length = e.Attribute.ArrowheadLength};
                }
                msaglGraph.Edges.Add(msaglEdge);
                msaglEdge.UserData = e;
                msaglEdge.LineWidth = e.Attribute.LineWidth;
            }
        }
        ...
        public static SugiyamaLayoutSettings CreateLayoutSettings(Graph graph) {
            ...
            if (settings != null) {
                ...
                settings.AspectRatio = graph.Attribute.AspectRatio;
                settings.MinimalWidth = graph.Attribute.MinimalWidth;
                settings.MinimalHeight = graph.Attribute.MinimalHeight;

                switch (graph.Attr.LayerDirection) {
                    case LayerDirection.LR:
                        if (settings.AspectRatio != 0)
                            settings.AspectRatio = 1/settings.AspectRatio;
                        settings.MinimalWidth = graph.Attribute.MinimalHeight;
                        settings.MinimalHeight = graph.Attribute.MinimalWidth;
                        break;
                    case LayerDirection.RL:
                        if (settings.AspectRatio != 0)
                            settings.AspectRatio = 1/settings.AspectRatio;
        ...
        public static Core.Layout.Node CreateGeometryNode(Graph drawingGraph, GeometryGraph geometryGraph, Node node,
            ...
            geomNode.Padding = node.Attribute.Padding;
            return geomNode;
        }
