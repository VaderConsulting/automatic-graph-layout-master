...
namespace Microsoft.Msagl.Drawing
{
    ...
    public class SvgGraphWriter
    {
        ...
        public void Write()
        {
            ...
            try
            {
                ...
                Open();
                WriteGraphAttribute(graph.Attr);
  ...
  //          const double scaleFromGdiToSvg = 1.5;
        ...
        static void WriteGraphAttribute(GraphAttribute graphAttr)
        {
        }
        ...
        void AddArrow(Point start, Point end, Edge edge)
        {
            ...
            DrawArrowPolygon(edge.Attribute, points);
        }
        ...
        void DrawPolygon(AttributeBase attr, Point[] points)
        {
            ...
            WriteStroke(attr);
            var edgeAttribute = attr as EdgeAttribute;
            ...
            {
                var NodeAttribute = attr as NodeAttribute;
                if (NodeAttribute != null)
                    WriteFill(NodeAttribute);
            }
        ...
        {
            NodeAttribute attr = node.Attribute;
            ...
            else
            {
                ...
                if (ellipse != null)//a bug when the axis are rotated
                    WriteEllipseOnPosition(node.Attribute, ellipse.Center, ellipse.AxisA.Length, ellipse.AxisB.Length);
                else
                {
        ...
        void WritePolyline(Polyline poly, NodeAttribute attr)
        {
            throw new NotImplementedException();
        }

        void WriteFillAndStroke(NodeAttribute attr)
        {
            WriteFill(attr);
            WriteStroke(attr);
        }
        ...
        {
            WriteStartElement("path");
            WriteFillAndStroke(node.Attribute);
        ...
        {
            NodeAttribute NodeAttribute = node.Attribute;
            ...
            DrawPolygon(node.Attribute, ps);
        }

        void WriteBox(Node node)
        {
            WriteStartElement("rect");
            WriteFill(node.Attribute);
            WriteStroke(node.Attribute);
            ...
            WriteAttribute("rx", node.Attribute.XRadius);
            WriteAttribute("ry", node.Attribute.YRadius);
            WriteEndElement();
        }
        ...
        void WriteEllipse(Node node)
        {
            ...
            WriteEllipseOnPosition(node.Attribute, center, rx, ry);
        }
        ...
        void WriteDoubleCircle(Node node)
        {
            ...
            var rx = geomNode.Width / 2;
            var ry = geomNode.Height / 2;
            WriteEllipseOnPosition(node.Attribute, center, rx * DoubleCircleOffsetRatio, ry * DoubleCircleOffsetRatio);
            WriteEllipseOnPosition(node.Attribute, center, rx, ry);
        }


        void WriteEllipseOnPosition(NodeAttribute NodeAttribute, Point center, double rx, double ry)
        {
            WriteStartElement("ellipse");
            WriteFill(NodeAttribute);
            WriteStroke(NodeAttribute);

            WriteFullEllipseGeometry(center, rx, ry);
            WriteEndElement();
        }

        void WriteFill(NodeAttribute attr)
        {
